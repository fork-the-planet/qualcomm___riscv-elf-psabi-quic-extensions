...
.2+| 195     .2+| `E_CALL_PLT`  .2+| Static   | _QC.EJ-Type_       .2+| 32-bit PC-relative function call (via PLT)
                                             <| S + A - P
.2+| 196     .2+| `ACCESS_16`   .2+| Static   | 16-Bit Instruction .2+| Relaxation Marker for 16-Bit Instruction (See Below).
                                             <| None
.2+| 197     .2+| `ACCESS_32`   .2+| Static   | 32-Bit Instruction .2+| Relaxation Marker for 32-Bit Instruction (See Below).
                                             <| None
.2+| 198-255 .2+| *Reserved*    .2+|          |                    .2+| Reserved for future use in the Qualcomm Extensions.
                                             <|
|===
...
The new instruction types starting _QC_ are defined in the `Xqci` specification cite:[xqcispec].

==== Relaxation Marker Relocations

We specify two "relaxation marker" relocations, which are used to mark instructions that are part of an access sequence, but that do not modify the relocated instruction during the relocation process.

These relocations are:
- `R_RISCV_QC_ACCESS_32` for 32-bit instructions; and
- `R_RISCV_QC_ACCESS_16` for 16-bit instructions.

NOTE: The design of these is based on the `R_RISCV_TPREL_ADD` relocation.

These are given for specific sizes so the linker knows the size of the underlying instruction that can be modified during relaxation.

The symbol and addend in these relocations should be the same as the prior relocation they relate to. If there is a non-zero offset in the access instruction, it should not be included in the access relocation addend.

[[sections]]
=== Sections

The Xqccmt extension re-uses the `.riscv.jvt` section defined in the RISC-V ELF psABI.
...
Which option of these to emit can be hard to evaluate within the compiler, as it depends on the layout of the executable, whether relaxation is enabled, and whether the load or store was compressed.
--

[%unbreakable]
--
Memory Accesses in the Medium Low Code Model which materialise the address for only use in the next access instruction should use `qc.e.li` plus a "relaxation marker" relocation on the access.
This can only be done if the `qc.e.li` and the access are adjacent, and the access is the only user of the address result of the `qc.e.li`.
For example:
...
----
    ...
    qc.e.li a0, symbol+addend               # R_RISCV_VENDOR QUALCOMM
                                            # R_RISCV_QC_E_32 symbol, addend
    # Load Value
    lw a0, 0(a0), %qc.access(symbol+addend) # R_RISCV_VENDOR QUALCOMM
                                            # R_RISCV_QC_ACCESS_32 symbol, addend
----
--
...
=== Xqccmt Loading and Handling

Xqccmt is a custom extension with a superset of the Zcmt functionality. It
shares instruction encodings, but interprets table entries slightly differently.

The Jump Table contents are emitted into a section called `+.riscv.jvt+`, as
described in <<sections>>. The start of the table is denoted with the
`+__jvt_base$+` symbol, which must be non-weak.

The program is responsible for loading `+__jvt_base$+` into the `jvt` CSR before
any jump table instructions are executed. This code should treat the
`+__jvt_base$+` symbol reference as weak, as the symbol will not be defined if
the linker does not create a jump table.

Existing Zcmt jump tables are compatible with Xqccmt, but Xqccmt jump tables are
not compatible with Zcmt. An executable can only contain one jump table -- so
the choice of which kind of jump table the linker should produce must be
explicit.
